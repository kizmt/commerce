...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const webhookSecret =
    process.env.SHOPIFY_WEBHOOK_SECRET || process.env.SHOPIFY_REVALIDATION_SECRET || '';
  ...
  const isAuthorized = isHmacValid || (legacySecret && legacySecret === process.env.SHOPIFY_REVALIDATION_SECRET);
  if (!isAuthorized) {
